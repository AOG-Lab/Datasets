...
.
```{r}
gset <- getGEO("GSE193391", GSEMatrix =TRUE, getGPL=FALSE) 
```
...
dummy = phdata[phdata$`diagnosis:ch1` %in% c("AD", "Control"),] #aqui se filtra por ALN y control


dummy$"diagnosis:ch1" = gsub("AD","disease", dummy$"diagnosis:ch1")
dummy$"diagnosis:ch1" = gsub("Control","control", dummy$"diagnosis:ch1")
...
```{r}
newGPL = gpltable[,c("ID","Probe.Label")]

dummy = separate(data = newGPL, col="Probe.Label",sep="//", into = c('x1','x2'),remove = TRUE)

dummy2 = dummy[,-3]

names(dummy2)[2]="GENE_SYMBOL"

mdata = merge(dummy2,exp,by.x="ID",by.y=0)
...
```{r}
p2g = data.frame(table(mdata$`GENE_SYMBOL`)) #cuanto de esos genesimbol se repiten
table(p2g$Freq)
```
...
```{r}
mdata$"GENE_SYMBOL" = gsub("-","_",mdata$"GENE_SYMBOL")
...
write.csv(ALZ3_sex_final_GSE195872, file = "AD_sex_GSE193391.csv", row.names = FALSE)
```
...
write.csv(ALZ3_GSE195873, file = "AD_GSE193391.csv", row.names = FALSE)
```
